...
func StartChat(hostUrl string, appid string, apiSecret string, apiKey string, question string, uid string, domain string) string {
	...
	go func() {
		data := genParams1(appid, question+",请简要回答，减少回复字数", uid, domain)
		conn.WriteJSON(data)
	...
	for {
		...
		if code != 0 {
			...
			return fmt.Sprintln(data["payload"])
		}
...
// 生成参数
func genParams1(appid, question string, uid string, domain string) map[string]interface{} { // 根据实际情况修改返回的数据结构和字段名
